...
abstract public class Serialize {
    ...
    static void serialize( CodeGen code ) {
        ...
        // Preserve which same-named structs are the source-level declarations.
        // Optimized graph types can carry sharper field states (e.g. len==0),
        // and cannot be used to reconstruct this mapping during loading.
        TreeMap<String,TypeStruct> named = new TreeMap<>();
        for( Type type : Parser.TYPES.values() )
            if( type instanceof TypeStruct ts )
                named.put(ts._name,ts);
        TypeStruct[] pubTypes = named.values().toArray(new TypeStruct[0]);

        // Compress into bytes
        BAOS baos = write(nodes, clz, pubTypes, deps, code._aliases, code._fidxs, code._rpcs, code._externFunc);
        ...
        if( false ) {
            ...
            BAOS baos2 = write(elf._nodes,elf._clz,elf._pubTypes,elf._deps, code._aliases, code._fidxs, code._rpcs, code._externFunc);
    ...
    // --------------------------------------------------
    static BAOS write(Ary<Node> nodes, TypeStruct clz, TypeStruct[] pubTypes, String[] depobjs, GlobalBits aliases, GlobalBits fidxs, GlobalBits rpcs, HashMap<Integer,String> externFunc) {
        ...
        for( Node n : nodes ) {
            ...
            if( n instanceof ParmNode parm ) parm._declaredType.gather(types);
            if( n instanceof TypeNode con ) con._con .gather(types);
        }
        // Only record canonical declarations already reachable from this
        // object's graph.  Pulling unrelated parser types into this closure
        // can include open or partially built types from other units.
        Ary<TypeStruct> writtenPubs = new Ary<>(TypeStruct.class);
        for( TypeStruct ts : pubTypes )
            if( types.containsKey(ts) )
                writtenPubs.add(ts);
        ...
        baos.packed2(writtenPubs._len);
        for( TypeStruct ts : writtenPubs )
            baos.packed4(types.get(ts));
    ...
    static TypeRead readTypes( CodeGen code, ElfReader elf, GlobalBits aliases, GlobalBits fidxs, GlobalBits rpcs ) {
        ...
        int npubTypes = bais.packed2();
        int[] pubTypeIds = new int[npubTypes];
        for( int i=0; i<npubTypes; i++ )
            pubTypeIds[i] = bais.packed4();
        ...
        elf._pubTypes = new TypeStruct[npubTypes];
        for( int i=0; i<npubTypes; i++ ) {
            TypeStruct ts = elf._pubTypes[i] = (TypeStruct)types[pubTypeIds[i]];
            Parser.TYPES.put(ts._name,ts);
        }
    ...
    static void readAll( CodeGen code, ElfReader elf, GlobalBits aliases, GlobalBits fidxs, GlobalBits rpcs ) {
        ...
        for( Node n : nodes ) {
            ...
            for( int i=0; i<len; i++ ) {
                ...
                Node def = idx==0 ? null : nodes.at(idx-1);
    ...
    private static boolean check( CodeGen code, Ary<Node> nodes ) {
        // Loaded nodes are complete, unlike nodes still being built by the
        // parser.  Recompute them with post-parse semantics.  The compilation-
        // unit Stop is the exception: its escape type depends on the complete
        // program's public interface, which ParseAll freezes only after every
        // compilation unit has been loaded.
        boolean good = true;
        CodeGen.Phase phase = code._phase;
        code._phase = CodeGen.Phase.Opto;
        try {
            for( Node n : nodes ) {
                if( n instanceof StopCUNode && !code.publicInterfaceFrozen() )
                    continue;
                Type nval = n.compute();
                if( nval != n._type ) {
        ...
        for( Node n : code._start._outputs ) {
            if( n instanceof ConstantNode con && !visit.get(con._nid) ) {
                for( Node use : con.outs() ) {
                    ...
                    // test as well.
                    if( use instanceof PtrToIntNode ptr )
                        for( Node add : ptr.outs() ) {
                            for( Node addUse : add.outs() )
                                conUsed(add,addUse,visit,cons);
                            conUsed(ptr,add,visit,cons);
                        }
            ...
            if( n.isConst() ) {
                ...
                for( Node cast : n._outputs )
                    if( cast != null && cast.nIns() == 2 && cast.in( 0 ) == null ) {
                        visit.set( cast._nid );
                        nodes.add( cast );
                        // Array pointer conversion adds the body offset after
                        // the zero-code PtrToInt.  Both nodes are global-value
                        // descendants of the constant and otherwise have no
                        // function-local root from which to walk them.
                        if( cast instanceof PtrToIntNode )
                            for( Node add : cast.outs() )
                                if( add instanceof AddNode && !visit.get(add._nid) ) {
                                    visit.set(add._nid);
                                    nodes.add(add);
                                }
                    }
    ...
    private static void printMulti(Node n, Ary<Node> rpo) {
        ...
        if( !(n instanceof StartNode) )
            Arrays.sort( rpo._es,rpo._len-n.nOuts(),rpo._len,
                         (x,y) -> x instanceof ProjNode px && y instanceof ProjNode py ? (py.idx() - px.idx()) : (y._nid - x._nid) );
        rpo.add(n);
    }
