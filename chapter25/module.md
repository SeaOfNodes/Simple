...
- - - (an exception is allowed for a golden instance in `<clinit>`)
...
and `exec'd`.  No files made, this is strictly a run and gun mode.
...
As before, since no module name is mentioned on the compile line, `Pet` becomes
both the module and a class.
