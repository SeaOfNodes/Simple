...
public class LoadRISC extends MemOpRISC {
    ...
    @Override int func3() {
        ...
        if( declType instanceof TypeFunPtr) func3 = 3; // 8 byte function pointers
        if( declType instanceof TypeConAry ary )
            func3 = switch( ary.log_size() ) {
            case 0 -> 4;         // LBU
            case 1 -> 5;         // LHU
            case 2 -> 2;         // LW
            case 3 -> 3;         // LD
            default -> throw Utils.TODO("RISC-V constant-array load for "+declType);
            };
        if( func3 == -1 ) throw Utils.TODO("RISC-V load for "+declType);
        return func3;
    }
