...
""");
        code.parse().opto();
        assertEquals("Stop[ return #2; return (Parm_x($fun17,i64,3)*x); return (Parm_x($fun18,i64,3)<<1); ]", code._stop.toString());
...
""");
        ...
        catch( Exception e ) { assertEquals("Might be null calling { i64 -> i64 #17}?",e.getMessage()); }
    }
