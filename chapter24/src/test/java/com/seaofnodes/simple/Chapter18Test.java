...
""");
        code.parse().opto();
        assertEquals("Stop[ return #2; return (Parm_x($fun20,i64,3)*x); return (Parm_x($fun21,i64,3)<<1); ]", code._stop.toString());
...
""");
        ...
        catch( Exception e ) { assertEquals("Might be null calling { i64 -> i64 #20}?",e.getMessage()); }
    }
